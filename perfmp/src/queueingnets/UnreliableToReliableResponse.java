...
public class UnreliableToReliableResponse extends Sim {
	...
	public UnreliableToReliableResponse(double d) {
		...
		QueueingNode service_mdw = new QueueingNode("SERVICE-MDW", trMsg_mediator, 1);		//bw between service and mediator
				
		//Mediator
		QueueingNode mediator_in = new QueueingNode("MEDIATOR-IN", prMsg, 1);
		QueueingNode mediator_out = new QueueingNode("MEDIATOR_OUT", trMsg_broker, 1);		//bw between mediator and broker
		
		//Broker
		QueueingNode broker_in = new QueueingNode("BROKER-IN", prMsg, 1);
		QueueingNode broker_out = new QueueingNode("BROKER-OUT", trMsg_app, 1);			//bw between broker and driver
		
		//Driver
		OnOffUQN driver_mdw = new OnOffUQN("DRIVER-MDW", prMsg, 1, ONOvrlDriver, OFFOvrlDriver, duration);		
		QueueingNode driver_app = new QueueingNode("DRIVER-APP", prMsg, 1);
		
		SinkLftLses sinkServiceApp = new SinkLftLses("SINK-SERIVCE-APP");		
		SinkLftLses sinkBrokerIn = new SinkLftLses("SINK-BROKER-IN");
		SinkLftLses sinkMediatorIn = new SinkLftLses("SINK-MEDIATOR-IN");
		SinkMdwLses sinkDriverMdw = new SinkMdwLses("SINK-DRIVER-MDW");
		SinkOvrlNet sinkDriverEnd = new SinkOvrlNet("SINK-SERVICE-APP");
	
		LftLsesBranch branchServiceApp = new LftLsesBranch(new Node[] {sinkServiceApp, service_mdw});
		...
		MdwLsesBranch branchDriverMdw = new MdwLsesBranch(new Node[] {sinkDriverMdw, driver_app});
	
		
		src.setLink(new Link(service_app));
		service_app.setLink(branchServiceApp);
		service_mdw.setLink(new Link(mediator_in));
		
		mediator_in.setLink(branchMediatorIn);
		mediator_out.setLink(new Link(broker_in));
		
		broker_in.setLink(branchBrokerIn);
		broker_out.setLink(new Link(driver_mdw));
		
		driver_mdw.setLink(branchDriverMdw);
		driver_app.setLink(new Link(sinkDriverEnd));
	...
	public void runSimulation(String responseTimesOutputFile, String successRateOutputFile) {
		...
		UnreliableToReliableResponse simulation = new UnreliableToReliableResponse(duration);
