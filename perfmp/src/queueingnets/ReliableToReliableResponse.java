...
public class ReliableToReliableResponse extends Sim {
	...
	public ReliableToReliableResponse(double d) {
		...
		//Service
		QueueingNode service_app = new QueueingNode("SERVICE-APP", trMsg_mediator, 1);		//bw between service and mediator
		...
		QueueingNode mediator_out = new QueueingNode("MEDIATOR_OUT", trMsg_broker, 1);		//bw between broker and mediator
		
		//Broker
		QueueingNode broker_in = new QueueingNode("BROKER-IN", prMsg, 1);
		QueueingNode broker_out = new QueueingNode("BROKER-OUT", trMsg_app, 1);		//bw between broker and app
				
		//Driver
		OnOffRQN driver_app = new OnOffRQN("DRIVER-APP", prMsg, 1, ONOvrlDriver, OFFOvrlDriver, duration);
		QueueingNode driver_mdw = new QueueingNode("DRIVER-MDW", prMsg, 1);		
		
		SinkLftLses sinkServiceApp = new SinkLftLses("SINK-SERVICE-APP");		
		SinkLftLses sinkBrokerIn = new SinkLftLses("SINK-BROKER-IN");
		SinkLftLses sinkMediatorIn = new SinkLftLses("SINK-MEDIATOR-IN");
		SinkOvrlNet sinkDriverEnd = new SinkOvrlNet("SINK-DRIVER-APP");
		...
		driver_mdw.setLink(new Link(driver_app));
	...
	public void runSimulation(String responseTimesOutputFile, String successRateOutputFile) {
		...
		ReliableToReliableResponse simulation = new ReliableToReliableResponse(duration);
