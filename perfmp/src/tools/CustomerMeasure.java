...
public class CustomerMeasure extends Measure {
	...
	public void addResponseTimeHoussam(double timestamp, double responseTime) {
	...
	public void saveResponseTimesCsv(String outputFile) {
		try {
            ...
            // Header column value
            String[] header = { "Timestamp", "ResponseTime"};
            write.writeNext(header);
            for (Double key : responseTimes.keySet()) {
            	String timestamp = key.toString();
            	String responseTime = responseTimes.get(key).toString();
            	String[] data = { timestamp, responseTime };
	...
	public void saveResponsesAndTimestamps() {
		try {
			...
			System.out.println(responseMeasures.length());
